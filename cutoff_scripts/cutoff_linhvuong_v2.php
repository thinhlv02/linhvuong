...
//die();
//select last record
$query_last_record = mysql_query("select a.id,a.time from linhvuongmobile_logs_2.`cutoff_chiso` a order by id desc limit 1", $conn_logs) or die("error last record: " . mysql_error());
...
#build
$query_userid_level_10 = mysql_query("select user_id from linhvuongmobile_2.tbl_quanphu a where a.level >= 10", $conn) or die('error build: ' . mysql_error());
...
#active
//select count(distinct(user_id)) as active from linhvuongmobile_logs_2.session_log a where date(a.logintime) = '2019-04-12'

$query_active = mysql_query("select count(distinct(user_id)) as active from linhvuongmobile_logs_2.session_log a 
    where date(a.logintime) ='" . $from . "' ", $conn_logs) or die("error active: " . mysql_error());
$row_active = mysql_fetch_array($query_active);
#active

#paycards: //select sum(a.`price`) price_pay_cards, SUM(a.`conversion_price`) price_pay_cards_cvs from linhvuongmobile_logs_2.pay_cards a where date(a.`requested_at`) =
//$query_pay = mysql_query("select sum(a.`price`) price_pay_cards, SUM(a.`conversion_price`) price_pay_cards_cvs from
//    linhvuongmobile_logs_2.pay_cards a where date(a.`requested_at`) =  '" . $from . "' ", $conn_logs) or die("error paycards: ". mysql_error());
...
$inapp_arr = mysql_query("select a.price, a.money from linhvuongmobile_2.tbl_product_id a group by price", $conn) or die('errror inapp arr' . mysql_error());
...
$query_in_logs = mysql_query("select xu,price from linhvuongmobile_logs_2.inapp_logs a 
...
//extra_gold
$query_extra_gold = mysql_query(" select ifnull(sum(a.`items_id`),0) extra_gold from linhvuongmobile_logs_2.`add_items_logs` a where date(a.`date`) =   '" . $from . "' ", $conn_logs) or die("error paycards: " . mysql_error());
...
FROM linhvuongmobile_logs_2.`ccu_log` t1
INNER JOIN
(
    SELECT `date`, MAX(total) AS max_total
    FROM linhvuongmobile_logs_2.`ccu_log`
    GROUP BY DATE(date)
) t2
    ...
    FROM
      ...
      FROM
        (SELECT 
          ...
          DATE(logintime) AS d 
        FROM
          linhvuongmobile_logs_2.session_log 
...
$str_insert = "
       INSERT INTO `linhvuongmobile_logs_2`.`cutoff_chiso` (
          `import`,
          `build`,
          `active`,
          `highest_online`,
...
$str_update = "
    ...
    UPDATE `linhvuongmobile_logs_2`.`cutoff_chiso` SET 
